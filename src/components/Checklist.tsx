...
import { ARRIVAL_CHECKLIST } from '../domain/checklist'
...
export function Checklist() {
  ...
  const total = ARRIVAL_CHECKLIST.length
  const count = ARRIVAL_CHECKLIST.filter((item) => done[item.id]).length
      ...
      <div className="checklist__head">
        <div>
          ...
          <p className="tile__hint">
            {count === total ? 'All set. Enjoy Oslo.' : `${count} of ${total} done`}
          </p>
        </div>
